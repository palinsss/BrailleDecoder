...
public class BrailleReader {
    ...
    private char[][] getBrailleChar(int position, int spacing, String[] brailleLine){
        // TODO: implementation
        if ((position + 1) * (WIDTH + spacing) - spacing > brailleLine[0].length()) {
            return null;
        }
        char[][]bitmap = new char[HEIGHT][WIDTH];
        for (int i = 0; i < HEIGHT; i++) {
            for (int j = 0; j < WIDTH; j++) {
                bitmap[i][j] = brailleLine[i].charAt(position * (WIDTH + spacing) + j);
            }
        }
        return bitmap;
    }
    ...
    public String translate(String[] brailleLine, char dotSymbol, int spacing){
        ...
        String result = "";
        int len = brailleLine[0].length() / (WIDTH + spacing) + 1;

        for (int i = 0; i < len; i++ ) {
            char[][]brailleChar = getBrailleChar(i, spacing, brailleLine);
            if (brailleChar == null){
                return "";
            }
            char dec = decoder.decodeBitmap(brailleChar, dotSymbol);
            if (dec == 0) {
                return "";
            }
            sb.append(dec) ;
